...
void rotate(int wheel, int direction){
	if(wheel == 1){
		...
		if(direction == 1){
			...
			if(wheel1[3] != wheel2[6]) rotate(2, -1);
		}
		else{
			char temp = wheel1.front();
			wheel1.pop_front();
			wheel1.push_back(temp);
			if(wheel1[1] != wheel2[6]) rotate(2, 1);
		}
	}
	else if(wheel == 2){
		if(visited[2] != 0) return;
		visited[2] = 1;
		...
		if(direction == 1){
			...
			if(wheel2[7] != wheel1[2]) rotate(1, -1);
			if(wheel2[3] != wheel3[6]) rotate(3, -1);
		}
		else{
			char temp = wheel2.front();
			wheel2.pop_front();
			wheel2.push_back(temp);
			if(wheel2[5] != wheel1[2]) rotate(1, 1);
			if(wheel2[1] != wheel3[6]) rotate(3, 1);
		}
	}
	else if(wheel == 3){
		...
		if(direction == 1){
			...
			if(wheel3[7] != wheel2[2]) rotate(2, -1);
			if(wheel3[3] != wheel4[6]) rotate(4, -1);
		}
		else{
			char temp = wheel3.front();
			wheel3.pop_front();
			wheel3.push_back(temp);
			if(wheel3[5] != wheel2[2]) rotate(2, 1);
			if(wheel3[1] != wheel4[6]) rotate(4, 1);
		}
	}
	else{
		if(visited[4] != 0) return;
		visited[4] = 1;
		...
		if(direction == 1){
			...
			if(wheel4[7] != wheel3[2]) rotate(3, -1);
		}
		else{
			char temp = wheel4.front();
			wheel4.pop_front();
			wheel4.push_back(temp);
			if(wheel4[5] != wheel3[2]) rotate(3, 1);
		}
	}
}
